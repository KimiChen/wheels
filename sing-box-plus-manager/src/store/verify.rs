...
pub struct Report {
    ...
    /// **结构**层面的漂移：列、约束与索引。表名对得上不等于结构对得上——
    /// 原来的门禁只比表名，加一列减一列两个方向都静默放行；
    /// 只比列的那一版又看不见 UNIQUE / CHECK / 索引（2026-09-20 补上）。
