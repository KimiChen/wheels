...
pub struct EntrySource {
    ...
    /// 看得见这种拨法所需的**档位级别**（`quota::level`）。缺省 0 = 人人可见。
    ///
    /// # 它只把链接藏起来，**不是拦截**
    ///
    /// 两种拨法**共用同一个 token**，差的只是前缀。所以拿着内网那条地址
    /// 把 `Lan-` 改成 `Wan-` 就能取到——`/sub/` 那一侧**不判级别**。
    ///
    /// 这是使用者明确选的口径（2026-09-20，与节点那条 `min_level` 同一条）：
    /// 分级是为了引导，不是为了防人。要真的挡住，在 `api::sub::build` 里
    /// 按同一个级别判一次、不够就回 404（与认不出 token 同一个答案，
    /// 403 等于告诉对方「这个前缀存在，只是你不够格」）——那一步没做。
    ///
    /// **别把它当访问控制用。** 哪天真需要挡住，改的是上面说的那一处，
    /// 而不是在页面上再藏一层。
    #[serde(default)]
    pub min_level: i64,
}
