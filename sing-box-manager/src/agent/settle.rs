...
use crate::agent::deploy::{atomic_replace, report, restore_active_after_failure, write_private};
...
pub async fn complete_barrier(
    ...
    if runtime.restart(&live, pb.new_epoch).await.is_err() {
        let r = restore_active_after_failure(pool, runtime, config_dir, "restart_failed").await?;
        bstore::delete_pending(pool, command_id).await?;
        return Ok(Some(r));
    }
    // 重启并健康后才记账 → 此后 active_revision==revision 作为幂等门真值。
    let out = match runtime.health_check(&pb.role).await {
        Ok(Health::Ok) => {
