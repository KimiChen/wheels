...
/// 当前 active revision 的完整本地记录。
pub async fn active(pool: &SqlitePool) -> Result<Option<LocalRevision>> {
    let row = sqlx::query(
...
/// 当前 active revision 的数据面角色；无 active revision 或旧记录缺少角色时返回 None。
pub async fn current_role(pool: &SqlitePool) -> Result<Option<String>> {
    let row = sqlx::query(
        "SELECT role FROM local_revisions WHERE active=1 ORDER BY revision DESC LIMIT 1",
    )
    .fetch_optional(pool)
    .await?;
...
/// 把某已存在 revision 置为唯一 active，并记录本次启动使用的新 epoch。
pub async fn activate(pool: &SqlitePool, revision: i64, epoch: i64) -> Result<()> {
    let mut tx = pool.begin().await?;
    sqlx::query("UPDATE local_revisions SET active=0")
        .execute(&mut *tx)
        .await?;
    sqlx::query(
        "UPDATE local_revisions SET active=1,runtime_epoch=?,applied_at=? WHERE revision=?",
    )
    .bind(epoch)
    .bind(crate::store::now_unix())
    .bind(revision)
    .execute(&mut *tx)
    .await?;
    tx.commit().await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn current_role_follows_active_revision() {
        let path = std::env::temp_dir().join(format!("sbm-role-{}.db", uuid::Uuid::new_v4()));
        let ps = path.to_string_lossy().to_string();
        let pool = open(&ps).await.unwrap();
        assert_eq!(current_role(&pool).await.unwrap(), None);
        record_applied(&pool, 1, "sha", "/tmp/config.json", "node", 1)
            .await
            .unwrap();
        assert_eq!(current_role(&pool).await.unwrap().as_deref(), Some("node"));
        pool.close().await;
        let _ = std::fs::remove_file(&path);
    }
}
