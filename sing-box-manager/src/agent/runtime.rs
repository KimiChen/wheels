...
impl ProcessRuntime {
    ...
    fn singbox_bin() -> String {
        std::env::var("SINGBOX_BIN").unwrap_or_else(|_| "sing-box".to_string())
    }
...
impl Runtime for ProcessRuntime {
    ...
    async fn health_check(&self, role: &str) -> Result<Health> {
        // 所有角色都必须由当前 Agent 持有一个仍存活的子进程。
        {
            let mut child = self.child.lock().unwrap();
            let Some(child) = child.as_mut() else {
                return Ok(Health::Down("没有受管 sing-box 进程".into()));
            };
...
impl Runtime for MockRuntime {
    ...
    async fn health_check(&self, role: &str) -> Result<Health> {
        self.calls.lock().unwrap().push(format!("health:{role}"));
        Ok(self
            .healths
            .lock()
            .unwrap()
