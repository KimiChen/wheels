...
pub async fn run() -> anyhow::Result<()> {
    ...
    // Agent 是 sing-box 进程所有者。服务重启后若本地已有 active revision，则恢复 live 配置；
    // systemd 应使用 KillMode=control-group，保证旧 Agent 退出时其子进程一并终止。
    if state::active_revision(&state_pool).await?.is_some() {
        let live = std::path::Path::new(&cfg.config_dir).join("config.json");
        if live.is_file() {
            let epoch = state::current_epoch(&state_pool).await?.unwrap_or(0);
            runtime::Runtime::restart(process_runtime.as_ref(), &live.to_string_lossy(), epoch)
                .await?;
        }
    }
    ...
    .await;
    // 正常信号退出和监听错误都先停止受管数据面；systemd KillMode=control-group 作为第二道保障。
