...
async fn status(State(st): State<AgentState>) -> Response {
    let current_revision = state::active_revision(&st.state).await.ok().flatten();
    let current_role = state::current_role(&st.state).await.ok().flatten();
    let singbox_running = match current_role.as_deref() {
        Some(role) => matches!(st.runtime.health_check(role).await, Ok(Health::Ok)),
        None => false,
    };
