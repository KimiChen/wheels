...
//! 身份名由不可变 (user_id,route_id) 确定性派生；SS uPSK / VLESS UUID 均信封加密保存。
...
/// 授权一条 Route：SS 生成 uPSK，VLESS 生成 UUID；明文均信封封存。
pub async fn grant_route(
    ...
    let (credential_kind, credential) = if entry.inbound_kind == "vless-reality" {
        ("user_route_uuid", uuid::Uuid::new_v4().to_string())
    } else {
        let method = entry.ss_method.as_deref().unwrap_or(NODE_SS_METHOD);
        ("user_route_upsk", generate_psk(method))
    };
    let now = now_unix();
    let mut tx = pool.begin().await?;
    let cid = secrets::put_psk_tx(&mut tx, cipher, credential_kind, &name, &credential).await?;
    sqlx::query(
...
pub async fn revoke_route(pool: &SqlitePool, user_id: &str, route_id: &str) -> Result<()> {
    ...
    secrets::delete_psk_tx(&mut tx, "user_route_uuid", &name).await?;
...
pub async fn eligible_desired(
    ...
    let rows = sqlx::query(
        "SELECT ur.identity_name, ur.upsk_credential_id FROM user_routes ur
         JOIN routes r ON r.id=ur.route_id JOIN entries e ON e.id=r.entry_id
         JOIN users u ON u.id=ur.user_id
         LEFT JOIN user_runtime_state urs ON urs.user_id=u.id
         WHERE r.entry_id=? AND ur.identity_name IS NOT NULL AND r.status='active'
           AND e.inbound_kind='shadowsocks'
           AND u.disabled=0 AND (u.expire_at IS NULL OR u.expire_at>?)
           AND COALESCE(urs.effective_disabled,0)=0
