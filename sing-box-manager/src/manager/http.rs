...
/// Controller 的唯一 HTTP 面：订阅、健康检查、就绪检查和 Prometheus 指标。
/// 拓扑、用户、发牌、发布、认证等管理操作只允许通过本机 CLI/配置完成。
pub fn controller_router(state: AppState) -> Router {
    let mut public = Router::new().route("/healthz", get(healthz));
    public = crate::subscription::add_routes(public);
    public = crate::manager::observ_http::add_public_routes(public);
...
mod auth_tests {
    ...
    async fn controller_app() -> (Router, SqlitePool) {
        std::env::set_var("ENCRYPTION_MASTER_KEY", STANDARD.encode([8u8; 32]));
        let path =
            std::env::temp_dir().join(format!("sbm-controllerhttp-{}.db", uuid::Uuid::new_v4()));
        let pool = store::open(&path.to_string_lossy()).await.unwrap();
        let state = AppState {
            pool: pool.clone(),
            cipher: Arc::new(Cipher::from_env(1).unwrap()),
            freshness_secs: 90,
            auth: Arc::new(crate::config::AuthConfig::default()),
            started_at: store::now_unix(),
        };
        (controller_router(state), pool)
    }
    ...
    #[tokio::test]
    async fn controller_exposes_no_management_routes() {
        let (app, pool) = controller_app().await;
        let health = app
            .clone()
            .oneshot(Request::get("/healthz").body(Body::empty()).unwrap())
            .await
        ...
        for path in ["/api/hosts", "/api/users", "/api/auth/login"] {
            let response = app
                .clone()
