...
pub fn host_publishable(input: &GateInput, now: i64, freshness_secs: i64) -> Readiness {
    ...
    // 首次部署尚无 live revision，sing-box 未运行是正常状态；已有 revision 才要求数据面健康。
    if input.has_current_revision && !input.singbox_running {
...
pub async fn gather(pool: &SqlitePool, host_id: &str) -> Result<GateInput> {
    ...
    Ok(GateInput {
        ...
        has_current_revision: agent.as_ref().and_then(|a| a.current_revision).is_some(),
...
mod tests {
    ...
    #[test]
    fn first_deploy_allows_singbox_not_running() {
        let now = 1_000_000;
        let mut input = green(now);
        input.has_current_revision = false;
        input.singbox_running = false;
        assert_eq!(
            host_publishable(&input, now, DEFAULT_FRESHNESS_SECS),
            Readiness::Publishable
        );
    }
}
