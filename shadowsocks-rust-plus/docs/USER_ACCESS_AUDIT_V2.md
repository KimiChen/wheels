...
>
> - 那份是**历史文档**，保存规范合同（v5，第 1–16 节）与第 1–8 轮审计、整改及 Linux 实装验收的
>   完整过程记录（第 17–30 节）。合同条文仍以那份为准，本文不复制、不改写合同。
> - 本文只列**尚未闭合**的条目。已闭合项不再重复，只在需要时引用其出处小节。
>
> 基线：overlay `main`，规范版本 v5，`patches/0003-user-audit.patch` 与
> `upstream.lock` 的 `prepared_tree_sha256` 一致。
...
### M-66（major）user_stats exporter 的 4 个用例在 Linux 上确定性失败

- **位置**：`crates/shadowsocks-service/src/server/user_stats.rs` 的
  `exporter_bounds_busy_response_workers`、`exporter_enforces_request_and_response_size_limits`、
...
- **现象**：以 root 运行时读取响应得 `ECONNRESET (104)`，而契约要求返回
  `413 + ERROR_REQUEST_TOO_LARGE`。
- **已排除**：不是并发抖动（单线程、空载、0.07 秒内确定性复现）；不是 root 专有（非 root 复跑
  同样 4 项失败，但失败位点不同，说明存在第二个成因）；**与审计功能无关**（仅
  `--features user-stats` 复跑同样失败）。这些用例在 macOS 上通过。
- **未定论**：具体机制。初步怀疑拒绝路径在客户端请求字节未读完时即关闭连接，Linux 因而发送
  RST 并使已写出的 413 丢失；一次半关闭实验未能干净证实，**故不作断言**。
- **影响**：使 §16 要求的 `cargo test --workspace --features user-audit` 在 Linux 上无法全绿。
- **未修复的原因（需决策，见第 5 节）**：修复点位于 `0001-eih-user-stats-http-unix-exporter.patch`
  引入的代码，而当前工作流只重新生成 `0003`；把 user-stats 的修复并入审计补丁属于交付边界决策。
- 出处：历史文档 §30.2。
...
| 项目 | 说明 | 阻塞因素 |
| --- | --- | --- |
| `cargo test --workspace --features user-audit`（Linux 全绿） | §16 验收项 | 被 `M-66` 阻塞 |
...
| D-5 | 发布构建对根 crate 用 `--features user-audit` 但**未加 `--no-default-features`**，默认 `full` feature 因而拉入 `reqwest`/`web-sys` 等审计功能并不需要的大量依赖，显著放大 D-4 | 评估是否收窄；若有意为之则在 §15.1 说明 |
...
1. **M-66 的归属**：修复点在 `0001` 范围。是（a）单独重新生成 `0001` 补丁，（b）并入 `0003`，
   还是（c）作为已知问题记录并在 §16 验收中豁免该 4 项？在决策前 `M-66` 不宜动手。
2. **D-5 是否收窄发布构建的 feature 集**：收窄可显著降低发布依赖面与下载量，但会改变发布产物
   的 feature 组合，属合同层面变更，需升版。
3. **实装节点的处置**：`10.0.1.3` 上 ssserver（`0.0.0.0:19991`）与 auditd 仍在运行，工作目录
   `/root/ss-audit.6lszxe`，另有 `/srv/ss-integ`、`/usr/local/bin/{ssserver,shadowsocks-auditd}`、
   `/etc/shadowsocks-{audit,rust-plus}`、两个 systemd unit 与
   `/usr/lib/{sysusers.d,tmpfiles.d}/shadowsocks-audit.conf`。是保留供后续验证，还是清理。
