...
### 2026-09-01 四轮复核：核实 m-236–m-244 的十一个提交，新增 m-245（已修复）

复核对象为二次/第三轮复核交付的 11 个提交（`0aba624`–`6d4cbd0`，含三个文档回写提交）。
**m-236–m-244 全部成立**，逐项核实：

- **哈希链与分区**：物化六个提交点（基线 `1e5d112` 起）的源码树逐对比较，四个补丁提交
  （m-236/m-237/m-238/m-239）各只动 `spool.rs` 一个文件；逐点 `prepared_tree_sha256` 与干净
  重放一致（`8d6f5b5b…`→`849ace0b…`→`523b96d9…`→`ee27bfbb…`）。
- **代码与文档事实复核**：m-236 的四个 `update_path_size` 调用点对照表逐点属实；m-237
  补回的四类断言与退化循环的摊平属实、无生产代码改动；m-238 的注释更正方向正确；m-239
  的前提（`non_gap_degraded` 的唯一读者是 `refresh_degraded_after_ack_locked`）属实；
  m-241/m-242/m-243 三处文档更正均属实——m-243 指出的基线误写源于任务书面的起始引用，
  实际复核范围自 `8b7b534` 起。
- **Linux 门禁**（Debian 13 / `10.0.1.3` / `release-toolchain.lock` 锁定的 rustc 1.97.0）：
  八条命令全 `EXIT=0`，计数与「本轮最终态验证」逐一对上（auditd 118、protocol 25、
  service 134/71、集成 4/4/1/1）。
- **变异检验独立复核**：m-236 删行转红；m-239 的四个调用点逐个 `true→false`，四条对应用例
  各转红（同事提交正文只记录了 tombstones 一处，本轮补齐四处的独立证据）；m-240/m-244 三条
  门禁的四个变异方向（锚点漂移、不存在的 `_locked` 函数名、无法解析的哈希、真实但不在
  当前历史上的悬空提交）全部转红。每条变异日志均确认 `Compiling shadowsocks-auditd`，没有
  重蹈第 2 节记录的二进制复用陷阱。
- **macOS** `verify.sh` 全绿（含 m-240/m-244 的三条新门禁）。
- m-240 的门禁在本轮被生产性触发一次：m-245 改补丁后、台账锚点同步前，`verify.sh` 红在
  `test_declared_anchor_prefix_matches_upstream_lock`。设计行为真实拦到一次漂移，记录在案。

**m-245（nit，已修复）m-239 补进 m-235 用例的注释块首行缩进错位**

- **位置**：`spool.rs` 用例 `post_commit_state_accounting_failure_degrades_without_stopping`
  内 m-239 注释块，首行 16 格、续行 12 格；同文件其余三处同源注释块均对齐。在一份靠注释
  比对判断同型性的文件里，错位会误导后续读者。
- **修复**：对齐缩进，无语义改动。**提交**：`a66291d`，锚点 `ee27bfbb…` → `250b6a7a…`。
- **验证**：Linux auditd 118 复跑绿；macOS `verify.sh` 通过。变异检验不适用（注释行）。



## 3. 待执行的验证

以下为发布前置。§16 已收窄为 v8 门禁并有 Linux 基线结果（见 §3.2）；m-229–m-245 新增的 Linux 用例与
最终态已由 2026-09-01 的四轮复核在 `10.0.1.3` 复跑全绿（见 §2 末三节与「本轮最终态验证」）。原始宽命令的失败记录见
§3.1；其余三项**从未在任何机器上执行过**：

| 项目 | 说明 | 阻塞因素 |
| --- | --- | --- |
| §16 收窄 Rust 门禁（v8：两条 workspace 命令 + ①②两类集成目标） | §16 验收项 | 已复跑：四轮复核在 m-245 最终态（`250b6a7a…`）八条命令全 `EXIT=0`（auditd 118、protocol 25、service 134/71、集成 4/4/1/1），**用的是 `release-toolchain.lock` 钉定的 rustc 1.97.0**；下次补丁变更后仍需再跑 |
...
- 2026-09-01：**四轮复核 m-236–m-244 的十一个提交**（`0aba624`–`6d4cbd0`）。逐点物化源码树
  核对哈希链与提交分区、在 `10.0.1.3` 用锁定的 rustc 1.97.0 复跑八条门禁全 `EXIT=0`
  （auditd 118、protocol 25、service 134/71、集成 4/4/1/1）、独立复核全部变异检验——m-236
  删行、m-239 四个调用点逐个 `true→false`（补齐同事正文只记一处的证据）、m-240/m-244 三条
  门禁的四个变异方向——均转红且逐条确认强制重建。结论：**全部成立**。新增并修复 `m-245`
  （nit，m-239 一处注释缩进错位，`a66291d`）。m-240 的锚点门禁在本轮被生产性触发一次
  （改补丁后锚点未同步，`verify.sh` 红），转绿后归档。最终态 `prepared_tree_sha256` 为
  `250b6a7a…`；发布前置仍是三项（fuzz 实跑、§14.5 压测、真实流量端到端），无变化。
