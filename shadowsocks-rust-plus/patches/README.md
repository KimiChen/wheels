...
`cargo check --all-targets`；target 缺失时脚本继续 feature-off/protocol/mock 检查并明确报告“未验证”，
设置 `SHADOWSOCKS_REQUIRE_AUDIT_TARGET=1` 才会 fail-closed。`prepare-source.sh` 在每个补丁应用前
