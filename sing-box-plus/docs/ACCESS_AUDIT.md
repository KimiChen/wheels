...
- `max_bytes` 是**逐文件**的，取值要跟着实际写入速率走，否则轮转形同虚设。
  逐用户分文件后实测每个身份约 1.4 MB/天，所以默认从 256 MiB 降到 16 MiB——
  沿用 256 MiB 会让一个身份半年才轮转一次，等于纪律 9/10 在生产上从不触发。
  **配了排除之后要再降一档**：生产实测排除规则把写入速率压到约 26 KB/天（降了约 100 倍），
  16 MiB 就变成一个文件横跨一年半，既碰不到轮转，也让「按时间段交付某人的记录」
  只能整文件给或自己按 `ts` 切。`config/server.example.json` 因此配 1 MiB，约 40 天一轮转
