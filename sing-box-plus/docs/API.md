...
| 413 | 请求行、请求头或请求体超限 |
| 413 之补充 | 请求体超限时服务端可能在客户端写完之前就判定并关闭。关闭一个接收队列里还有数据的 socket 会发 RST，把已写回的 413 一并丢掉，客户端于是看到「连接被重置」而不是状态码。服务端做了有界的 lingering 排空来缩小这个窗口，但**消不掉**它——客户端必须把「连接被重置」与错误码同等对待 |
