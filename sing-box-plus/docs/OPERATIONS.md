...
1. **额度是纯内存的**，进程重启即全部解封，直到 collector 按新 `runtime_id` 重推。
   `PUT /v3/quota` 返回的 409 是这条义务的兜底信号。
...
**闸断是事后的，会超额一小段。** 计数回调在字节已经过去之后才跑，跨过零点的那一次传输
已经发生了；并发连接越多超出越多，上限约为「并发连接数 × copy 分块」。
生产实测：1 GiB 额度、多连接并发，实际停在 1 073 853 248 字节，超额 111 424 字节（0.0104%）。
对外的计费说明里不要写「一字节不超」。
...
| 症状 | 首先检查 |
| --- | --- |
| 快照 404 | 采集器是否还在打旧路径。本项目只有 `/v3/snapshot`，`/v1`、`/v2` 恒为 404 |
| 快照 429 | 并发上限；采集端应重试且**不得入账** |
| `health` 前三位有真 | 都是粘滞的。`identity_limit_reached` 意味着该 runtime 余下时间全部不可入账，必须走计划重启 |
| `health.audit_dropped` 为真 | **账照常入，审计有缺口**。粘滞，重启才清。查进程日志里的「访问审计丢弃」行看原因与累计次数：多半是磁盘满、目录权限被改，或纪律 10 触底而没有已轮转文件可删（此时上限根本兑现不了，要么调大 `max_total_bytes`，要么调小 `max_bytes` 让轮转更早发生）。它**不会**让 `/healthz` 变 503，这是刻意的 |
