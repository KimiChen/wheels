...
go build ./...
# 非 Unix 平台也必须能编译。listen_other.go 里那条「本项目只支持 Unix」的明确报错，
# 只有在 !unix 构建得起来时才到得了；而带 //go:build unix 的常量一旦被无 tag 的文件
# 引用，本机 go test 永远看不见，唯有交叉编译能发现。
GOOS=windows GOARCH=amd64 go build -tags "$tags" ./...
