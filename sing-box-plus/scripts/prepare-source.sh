...
# 本项目是零补丁 wrapper（D1）：patches/ 不存在是正常状态。
# 一旦按 §4.7 末段的触发条件引入补丁，这里必须零 fuzz 重放 series。
series="$SING_BOX_PLUS_ROOT/patches/series"
if [[ -f "$series" ]]; then
  require_command patch
  while IFS= read -r patch_name; do
    [[ -z "$patch_name" || "$patch_name" == \#* ]] && continue
    [[ "$patch_name" != */* && "$patch_name" == *.patch ]] || die "非法补丁名：$patch_name"
    patch_path="$SING_BOX_PLUS_ROOT/patches/$patch_name"
