...
1. **长跑期间轮转从未触发**：7 天结束时最大的审计文件是 15.02 MB，`max_bytes` 是 16 MiB，
   差 1.7 MiB 没跨过去。因此纪律 9（inode 检测）与纪律 10（总量封顶）在生产上**一次都没跑过**，
   目前只有单元测试覆盖。要真正验证，把 `max_bytes` 临时调小一档再跑一段即可。
