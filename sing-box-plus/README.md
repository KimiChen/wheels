...
- **采集端持久化「推进基线」与「产出计费记录」两步的先后同样是一次显式取舍**，不得由代码顺序
  偶然决定。先产出计费记录、后推进基线时，两步之间崩溃会让下游看到已计费的一批而基线仍停在
  上一个 `sequence`，重启后同一段字节被重新算成一批发出去——**重复计费**；反过来则是状态已推进
  但少一条批次记录——**漏记一批**。幂等批次 ID 挡不住前者：重放出来的批次 `sequence` 与增量都不同，
  batch id 自然也不同。参考实现选择后者（先推进基线），即与 `baseline` 策略同向的「宁可漏记、
  不可重复」。要两者都不发生就需要把两步做成一次原子提交。
...
| 5 长跑与结算验证 | ✅（带保留） | **7 天 7 小时连续运行完成**（2026-09-07 00:32 ~ 09-14 17:44，真实用户流量）：10 453 轮采集、10 453 次额度下发、合计入账 19.2 GiB，**四项判据（负增量 / 未知 runtime / 重复 sequence / unhealthy 入账）全为 0**。审计侧 65 747 条记录、0 半行、0 事件行、0 混入他人、`seq` 逐文件连续。保留项见 §7.1 末段：轮转未触发、跨两个构建、流量不均 |
...
1. **轮转只验证了一半**：2026-09-15 11:01:47 UTC 生产上首次触发轮转（`kimi-vless` 的文件写到
   16 777 247 字节越过 16 MiB），重命名 + 重开正常，`seq` 69833→69834 无缺无重，
   旧文件停止增长。但那是**内部**轮转走的 `rotate()`；纪律 9 真正要防的是**外部**改动
   （logrotate 或人手 `rm`）后 fd 落进已删 inode，那条 `os.SameFile` 检测路径至今没被触发过。
   纪律 10（`max_total_bytes` 2 GiB）同样未触发——目录总量才 18 MB。两者仍只有单元测试覆盖。
