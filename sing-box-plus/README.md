...
（§5.1 的基线键以 `node_id` 打头，否则 `server_id` 命名空间会跨实现串号）。两者的 SS 配置
...
用户集合的变更（增、删、停用、凭据轮换）在 v1 中通过修改配置并受控重启生效，走 §5.3 的流程。
...
可直接作为本项目的契约测试与参考采集器。sing-box 特有信息以附加字段追加（现有校验器忽略
...
用户按 `name` 再按 `generation` 排序。

`inbound_type` 为**必填**（取 `metadata.InboundType`，首期取值 `vless` 或 `shadowsocks`）：首期已同时
支持两类协议，仅凭 `server_id` 无法区分；但它只是展示与运维维度，**不进入 §5.1 的基线键**，
以保持与 v1 契约的可复用性。`tcp_sessions`、`udp_sessions` 是当前活跃数、不参与结算，
供计划重启时判断是否已排空（§5.3）。
...
2. 启用统计时，逐 inbound 校验并在任一不满足时**启动失败**，不以部分覆盖或零归属模式运行：
   ...
   - `server_id`（:= inbound `tag`）非空且在节点内唯一；
...
instance.Router().AppendTracker(userstats.NewTracker(registry, logger))
...
| 配置与失败关闭 | §4.6 全部 10 条校验路径与错误信息（含 SS 特化校验、SSM 两层互斥、上游账本互斥） | 是 | 1–2 人周 |
...
必需项合计约 **14–25 人周**，另加 15–25% 的复核返工缓冲；含可选的重载对账与排空为 15–27 人周。
...
| 3 | 四向 tracker / registry | `go test -race` 全绿，含 `New → AppendTracker → Start` 全路径与“`Start` 后追加必被 `-race` 报出竞争”的负向用例；四向 oracle 误差 = 0；Linux amd64 真实 splice 用例覆盖；多 tracker 叠加 unwrap 断言通过；§4.6 十条失败关闭路径全部有用例 |
...
| # | 事项 | 选项 | 当前取值 | 决策时点 |
| --- | --- | --- | --- | --- |
| D3 | 快照 schema | 复用 `shadowsocks-rust-plus` v1 形状 / 另立 v2 | 复用 v1（`inbound_type` 为必填但不进基线键，§4.5） | 里程碑 4 之前 |
| D5 | 是否提供热用户增删接口 | 不提供（v1 只读，用户变更走受控重启或 SIGHUP 重载）/ 另开受控写入端点 | 不提供 | 里程碑 6 之后评估 |
