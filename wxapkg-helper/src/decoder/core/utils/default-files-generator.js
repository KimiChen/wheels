...
import { isPluginPath, printLog, replaceExt } from './common.js';
import { localFileExists, readLocalJsonFile, saveLocalFile } from './fs-process.js';
/**
 * 默认文件生成工具类
 * 负责生成小程序项目的默认文件结构
 */
export class DefaultFilesGeneratorUtils {
    ...
    static async analyzeComponentDependencies(outputPath, analysisList, deps = []) {
        ...
        for (const pagePath of analysisList) {
            ...
            if (localFileExists(pageJsonPath)) {
                try {
                    ...
                    if (componentPaths.length > 0) {
                        const newDeps = await DefaultFilesGeneratorUtils.analyzeComponentDependencies(outputPath, componentPaths, allDeps);
    ...
    static async generateDefaultAppFiles(outputPath) {
        ...
        const allPage = await DefaultFilesGeneratorUtils.analyzeComponentDependencies(outputPath, appConfigPages);
    ...
    static generatePageFiles(outputPath, pagePath, options = {}) {
        const { generateJson = true, generateJs = true, generateWxml = true, generateWxss = false, jsContent = "Page({ data: {} })", jsonContent = '{\n  "component":true\n}', wxmlContent = `<text>${pagePath}</text>`, wxssContent = `/* ${pagePath} styles */` } = options;
        if (generateJson) {
