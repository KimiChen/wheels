                    ...
                    if (dec.init.type == "CallExpression") {
                                    ...
                                    for (let x of dec.init.arguments[1].elements) {
                                        ...
                                        if (!v && typeof v != "number") {
                                            if (x.type == "UnaryExpression" && x.operator == "-")
                                                ...
                                                throw Error("Unknown type of object in _m attrs array: " + x.type);
                                        }
                                        if (name === null) {
                                            name = v;
                                        }
                                        ...
                                        if (e.type == "VariableDeclaration") {
                                            for (let f of e.declarations) {
                                                if (f.init.type == "LogicalExpression" && f.init.left.type == "CallExpression") {
                                                    if (f.init.left.callee.name == "_1")
                                                        data = z[f.init.left.arguments[0].value];
                                                    else if (f.init.left.callee.name == "_1z")
                                                        data = z[zMulName][f.init.left.arguments[1].value];
                        ...
                        if (dec.init.object.type == "MemberExpression" && dec.init.object.object.name == "e_" && dec.init.object.property.type == "MemberExpression" && dec.init.object.property.object.name == "x") {
                            if (dec.init.property.name == "j") { //include
                                ...
                                throw Error("Unknown member expression declaration.");
                        }
                        else
                            throw Error("Unknown member expression declaration.");
                    }
                    else
                        throw Error("Unknown declaration init type " + dec.init.type);
                }
                break;
            case "IfStatement":
                if (e.test.callee.name.startsWith("_o")) {
                    function parse_OFun(e) {
                        if (e.test.callee.name == "_o")
                            return z[e.test.arguments[0].value];
                        else if (e.test.callee.name == "_oz")
                            return z[zMulName][e.test.arguments[1].value];
                        else
                            throw Error("Unknown if statement test callee name:" + e.test.callee.name);
                    }
...
function wxmlify(str, isText) {
    ...
    if (isText)
        return str; //may have some bugs in some specific case(undocumented by tx)
    else
...
function elemToString(elem, dep) {
    ...
    if (elem.tag == "block") {
        if (elem.son.length == 1 && !isTextTag(elem.son[0])) {
            ...
            let ret = [];
            for (let s of elem.son)
                ret.push(elemRecursion(s, dep));
    ...
    if (elem.son.length == 0) {
        if (longerList.includes(elem.tag))
            return ret + " />\n";
        else
            return ret + "></" + elem.tag + ">\n";
    }
    ...
    let rets = [ret];
    for (let s of elem.son)
        rets.push(elemRecursion(s, dep + 1));
...
function getDecompiledWxml(code, z, xPool) {
    ...
    for (let elem of r.son)
        ans.push(elemToString(elem, 0));
    return ans.join("");
}
export function tryDecompileWxml(f_func_code, z, define, xPool) {
    try {
