...
import { listLocalFiles, readLocalFile, saveLocalFile } from "../utils/fs-process.js";
import { appJsonExcludeKeys, cssBodyToPageReg, pluginDirRename } from "../constant/index.js";
import { getZ } from "../utils/get-z.js";
import { tryDecompileWxml } from "../utils/decompile-wxml.js";
import { arrayDeduplication, getParameterNames, isPluginPath, isWxAppid, jsBeautify, printLog, removeElement, resetPluginPath, resetWxsRequirePath, sleep, toPosixPath } from "../utils/common.js";
import { getAppPackCodeInfo } from "../utils/get-pack-code-info.js";
...
export class AppDecompilation extends BaseDecompilation {
    ...
    isStyleRuntimeCode(code) {
        return (code.includes('setCssToHead') ||
    ...
    canResolveGwxRuntimeCode(code) {
        ...
        return Boolean(Object.keys(ALL_ENTRYS).length ||
    ...
    async decompileAppJSON() {
        ...
        if (appConfig.entryPagePath)
            appConfig.entryPagePath = appConfig.entryPagePath.replace('.html', '');
        ...
        if (appConfig.tabBar) {
            ...
            const allFileBufferInfo = listLocalFiles(this.pathInfo.outputPath)
                .map(filePath => {
                return {
            ...
            appConfig.tabBar.list = appConfig.tabBar.list.map((info) => {
                ...
                if (info.iconData) {
                    ...
                    if (found)
                        result.iconPath = toPosixPath(path.relative(this.pathInfo.outputPath, found.path));
                }
                if (info.selectedIconData) {
                    const found = allFileBufferInfo.find(item => item.data === info.selectedIconData);
                    if (found)
                        result.selectedIconPath = toPosixPath(path.relative(this.pathInfo.outputPath, found.path));
                }
                return result;
            });
        }
        ...
        for (const filePath in __wxAppCode__) {
            ...
            for (const key in usingComponents) {
                ...
                if (!isPluginPath(usingComponents[key])) {
                    usingComponents[key] = toPosixPath(path.relative(dir, path.join(dir, usingComponents[key])));
                }
            }
    ...
    _decompilePluginAppJS(vm, plugins) {
        ...
        for (const pluginName in plugins) {
            ...
            pluginDefine = function (name, func) {
                const pluginPath = toPosixPath(path.relative(_this.pathInfo.outputPath, _this.pathInfo.resolve(`${pluginDirRename[0]}/${appid}/${name}`)));
                ...
                if (Array.isArray(item)) {
                    ...
                    if (type === 0) {
                        ...
                        if (typeof item[1] === 'string') {
                            const relativePath = toPosixPath(path.relative(this.pathInfo.outputResolve(path.dirname(cssPath)), this.pathInfo.resolve(item[1])));
    ...
    async decompileAppWXSSWithRpx() {
        ...
        do {
            ...
            const cssSeedVm = createVM({
                sandbox: {
                    setCssToHead: this._setCssToHead.bind(this)
                }
            });
            runVmCode(cssSeedVm, cssSeedCode);
        } while (lastMatch);
        /* 拦截组件的 css */
        const __wxAppCode__ = vm.sandbox['__wxAppCode__'];
        for (let cssPath in __wxAppCode__) {
            if (path.extname(cssPath) !== '.wxss')
                continue;
    ...
    async decompileAppWXSS() {
        ...
        Array.from(children).forEach((styleEl) => {
            if (this.packType !== 'main') {
                if (mainPackageRenderedNodes.includes(styleEl))
                    return;
            }
            const wxss_path = styleEl.getAttribute('wxss:path');
            if (['', 'null', 'undefined', undefined, null].includes(wxss_path))
                return;
    ...
    functionToWXS(wxsFunc, basePath) {
        ...
        code = code.replace(wxsCodeRequireReg, (matchString) => {
            ...
            let relativePath = toPosixPath(path.relative(this.pathInfo.resolve(path.dirname(basePath)), this.pathInfo.resolve(newRequireString)));
    ...
    executeAllGwxFunction(code) {
        code = code
            .replaceAll('var e_={}', `var e_ = {}; window.COMPONENTS = global;`)
            .replaceAll('function(){if(!nnm[n])', `function(){ return {n, func: nnm[n]};`);
        const vm = createVM({
            sandbox: {
                setTimeout
            }
        ...
        const ALL_MODULES = {};
        const ALL_DEFINES = {};
        const pluginRoot = this.pathInfo.resolve(pluginDirRename[1]);
        const pluginNames = [...new Set(listLocalFiles(pluginRoot)
            .map(filePath => path.relative(pluginRoot, filePath).split(path.sep)[0])
            .filter(isWxAppid))];
    ...
    async decompileAppWXS() {
        ...
        // 解析模板归属
        wxsRefInfo.forEach(item => {
            let relativePath = toPosixPath(path.relative(this.pathInfo.resolve(path.dirname(item.wxmlPath)), this.pathInfo.resolve(item.wxsPath)));
            if (item.isInline) {
                    ...
                    if (matchAppidInfo && wxmlPath.includes(pluginDirRename[0])) {
                        ...
                        const srcList = Array.from(allImageEls).map(node => {
                            ...
                            if (src.startsWith('/')) {
                                ...
                                src = toPosixPath(path.relative(path.dirname(wxmlPath), src));
                            }
