...
> 状态：P2 完整采集体验已落地，2026-09-26 更新。Linux 采集器、WSS 通道、
> 节点凭据、FRP 连接适配器、TCP 探测闭环、SQLite 分钟聚合历史与累计流量、
> 趋势与恢复均已实现，`scripts/verify.sh` 含回环端到端冒烟（上报/探测/历史/
> 重启恢复/隧道转发；非 Linux 平台指标组按契约降级为未知）。隧道对账展示、
> 权限/字段裁剪强化与发布完善在 P3。
