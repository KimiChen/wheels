...
上游基线从 `ab99d56e` 更新到 `0116c5a1`（更新时的 `main`），保留 `.kim`
版本后缀、流量统计、定制页面、公开路由限制和精简 HTML 配置注入。新增的渠道监控
用户排名隐藏开关已接入精简配置注入，并覆盖启用与省略场景的回归测试。
上游已包含的翻译补全退出 Overlay。构建脚本为嵌套 pnpm 命令设置锁定工具路径，
保证新增的翻译完整性检查也使用 `pnpm@9.15.9`。

本次新增三个 SQL 迁移，未修改历史 SQL 迁移：

- `235_group_model_allowlist.sql`：将 `groups.models_list_config` 重命名为
  `model_allowlist`；配置从仅控制模型展示扩展为同时限制请求准入。
- `236_group_model_allowlist_repair.sql`：补齐或修复白名单列，保留已有配置。
- `237_add_minimax_platform.sql`：扩展平台 CHECK 约束，允许 MiniMax。

**回滚注意：** 白名单迁移重命名了旧程序依赖的列，不能仅切换到 `0.2.1.kim`
二进制回滚。发布前应验证数据库备份；需要回退数据库时，按下文流程先保全现状，
再审批恢复方案。已配置模型列表的分组应在发布前检查白名单是否覆盖实际请求模型。

新增可选配置 `ops.cleanup.system_log_retention_days`，默认 30，启用运维清理时
必须大于 0；错误日志与指标保留天数为 0 现在表示清空相应数据，不能视为禁用清理。
固定了上游允许域名列表的环境若使用 MiniMax 国际站，需在私有配置中加入其官方 API
主机。Go Redis 依赖更新到 `v9.22.0`。

本次验证：Overlay 树校验、`go test ./...`、公开配置注入的 `unit` 回归测试、
前端 lint 与类型检查、14 个关键测试文件（177 项）、Linux amd64 发布构建和
`SHA256SUMS` 校验通过。本机未安装 `golangci-lint`，未执行后端 lint。
