...
func NewFrontendServer(settingsProvider PublicSettingsProvider) (*FrontendServer, error) {
	...
	appHTML, err := readEmbeddedHTML(distFS, "app.html")
	if err != nil {
		appHTML = baseHTML
	}
	...
	appCache := NewHTMLCache()
	appCache.SetBaseHTML(appHTML)
...
func (s *FrontendServer) InvalidateCache() {
	...
	if s != nil && s.appCache != nil {
		s.appCache.Invalidate()
	}
}
...
func (s *FrontendServer) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// For index/app HTML or SPA routes, serve with injected settings.
		if cleanPath == "index.html" || !s.fileExists(cleanPath) {
			s.serveHTML(c, s.baseHTML, s.cache)
			return
		}
		if cleanPath == "app.html" {
			s.serveHTML(c, s.appHTML, s.appCache)
			return
		}
...
func readEmbeddedHTML(distFS fs.FS, name string) ([]byte, error) {
...
func (s *FrontendServer) serveHTML(c *gin.Context, baseHTML []byte, cache *HTMLCache) {
...
func injectSettingsIntoHTML(baseHTML []byte, settingsJSON []byte) []byte {
...
func injectSiteTitle(html, settingsJSON []byte) []byte {
	...
	newTitle := []byte("<title>" + htmlpkg.EscapeString(cfg.SiteName) + " - Secure Portal</title>")
	var buf bytes.Buffer
