...
// Sparse-payload mapping: enabled (default) omits the key and the frontend
// default applies; only explicit "false" is emitted.
func TestSettingService_GetPublicSettingsForInjection_MirrorsSubscriptionEnabled(t *testing.T) {
	for _, value := range []string{"false", "true"} {
		...
		require.True(t, ok)
		got, present := payload["subscription_enabled"]
		if value == "true" {
			require.False(t, present, "value=%q", value)
		} else {
			require.True(t, present, "value=%q", value)
			require.Equal(t, false, got, "value=%q", value)
		}
	}
}
...
func TestSettingService_GetPublicSettings_PaymentBalanceDisabledStrictTrue(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			payload, ok := raw.(PublicSettingsInjectionPayload)
			require.True(t, ok)
			got, present := payload["payment_balance_disabled"]
			if tc.want {
