...
const appStoreState = vi.hoisted(() => ({
  ...
  getPublicSettings: (): Record<string, unknown> | undefined => undefined,
}))
...
vi.mock('@/stores', async () => {
  ...
  appStoreState.getPublicSettings = () => state.cachedPublicSettings
  return {
    useAppStore: () => ({
      showError,
      showInfo,
...
async function mountSubscriptionConfirm(options: Parameters<typeof checkoutInfoWithPlansFixture>[0] = {}) {
  ...
  // Fork note: missing subscription_enabled resolves to hidden (sparse SSR config);
  // the real store defaults it to true, so mount helpers mirror that default unless the
  // test pre-set the flag.
  if (!appStoreState.getPublicSettings()) {
...
describe('PaymentView subscription feature flag', () => {
  ...
  it('keeps the top-up / subscribe switcher when subscription_enabled is enabled (store default)', async () => {
